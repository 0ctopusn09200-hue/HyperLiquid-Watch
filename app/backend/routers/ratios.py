...
router = APIRouter(
    prefix="/long-short-ratios",
    tags=["long-short-ratios"]
)
...
async def get_current_ratios(
    ...
    """Get current long/short ratios (REALTIME from liquidations table)"""
    ratios = ratio_service.get_current_ratios(db, coin)

    return ApiResponse(
        data={"ratios": [r.model_dump() for r in ratios]}
    )
...
async def get_ratio_history(
    ...
    """Get long/short ratio history (minute buckets from liquidations)"""
